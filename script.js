...
// Параллакс эффект для hero секции (только на desktop)
if (!isMobile()) {
    window.addEventListener('scroll', throttle(() => {
...
function loadSiteData() {
    ...
    if (savedData) {
        try {
            ...
            // Обновляем особенности (только если есть данные и элементы в HTML пусты)
            const featuresGrid = document.querySelector('.features-grid');
            if (featuresGrid && siteData.features && siteData.features.length > 0) {
                // Обновляем только если админка изменила данные
                const hasExistingFeatures = featuresGrid.querySelectorAll('.feature-card').length > 0;
                if (hasExistingFeatures) {
                    // Не перезаписываем существующий HTML
                    console.log('Features already exist in HTML, skipping override');
                } else {
            ...
            if (googlePlayBtn && siteData.links && siteData.links.stores && siteData.links.stores.googlePlay) {
                googlePlayBtn.href = siteData.links.stores.googlePlay;
                googlePlayBtn.setAttribute('target', '_blank');
                googlePlayBtn.setAttribute('rel', 'noopener noreferrer');
                console.log('Google Play ссылка установлена:', siteData.links.stores.googlePlay);
            }

            if (appStoreBtn && siteData.links && siteData.links.stores && siteData.links.stores.appStore) {
                appStoreBtn.href = siteData.links.stores.appStore;
                appStoreBtn.setAttribute('target', '_blank');
                appStoreBtn.setAttribute('rel', 'noopener noreferrer');
                console.log('App Store ссылка установлена:', siteData.links.stores.appStore);
            }
...
window.addEventListener('load', () => {
    ...
    if (!isMobile()) {
        initTechBackground(); // Инициализируем технологичный фон только на desktop
        initAdvancedAnimations(); // Инициализируем дополнительные анимации
    }

    initScrollProgress(); // Инициализируем прогресс-бар (легкий)
});
...
function initAdvancedAnimations() {
    ...
    // Parallax эффект для орбов (только на desktop, с throttle)
    if (!isMobile()) {
